...
en_fall_2020 <- raw %>%
  clean_names()%>%
  select(course_id, u_grad, course_name, course_title, instructor_full_name)%>%
  group_by(course_title)%>%
  filter(course_title == "GOV 50")%>%
  summarise(count = sum(u_grad, na.rm = 1))
en_fall_2020
...
biggest_10_courses <- raw %>%
  ...
  arrange(desc(count))%>%
  slice(1:10)
ggplot(biggest_10_courses, aes(x = course_title, y = count)) +
  geom_col() + 
  coord_flip() +
  labs(title = "Ten Biggest Undergrad Courses at Harvard", x = "Course Title", y = "Enrollment")
```
